...
public class VNectBarracudaRunner : MonoBehaviour
{
    ...
    private float totalAverageScore = 0.0f;
    private int updateCount = 0;
    private float totalFrameTime = 0.0f; // Загальний час обробки усіх кадрів
    private int frameCount = 0; // Кількість оброблених кадрів
    private Stopwatch stopwatch = new Stopwatch(); // Таймер для вимірювання часу обробки
    ...
    private void Update()
    {
        if (!Lock)
        {
            stopwatch.Reset(); // Скидання таймера
            stopwatch.Start(); // Запуск таймера перед обробкою кадру
            ...
            {
                totalScore += jointPoints[j].score3D;
            }

            float averageScore = totalScore / JointNum; 
            totalAverageScore += averageScore; 
            updateCount++;

            stopwatch.Stop(); // Зупинка таймера після обробки кадру
            float frameTime = (float)stopwatch.Elapsed.TotalMilliseconds; // Час обробки одного кадру в мілісекундах
            totalFrameTime += frameTime; // Додавання часу кадру до загального часу
            frameCount++; // Збільшення лічильника кадрів
        ...
        if (updateCount > 0)
        {
            float cumulativeAverageScore = totalAverageScore / updateCount;
            float averageFrameTime = totalFrameTime / frameCount; // Середній час обробки кадру
